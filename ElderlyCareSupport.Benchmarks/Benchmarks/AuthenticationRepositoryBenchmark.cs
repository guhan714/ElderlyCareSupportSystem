...
public class AuthenticationRepositoryBenchmark : GlobalSetup
{
    ...
    {
        using var dapperDbContext = new DapperDbContext(_connectionString);

        var connection = dapperDbContext.CreateConnection();
        var result = await connection.QueryFirstOrDefaultAsync<UserDto>("""SELECT u."Username", u."Email", u."PasswordHash", r."Name" AS Role FROM "Users" AS u INNER JOIN "Roles" AS r ON u."RoleId" = r."Id" WHERE u."Username" = @userName;""", new { userName = _userName});
        return result;
    }
